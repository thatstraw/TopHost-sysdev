...
class CWidgetFieldColumnsList extends CWidgetField {
	...
	public function validate(bool $strict = false): array {
		...
		foreach ($columns as $column_index => &$value) {
			...
			$field = (new CWidgetFieldTimePeriod($this->name.'.'.$column_index.'.time_period',
				'/'.($column_index + 1)
			))
				->setDefault([
			...
			if (array_key_exists('time_period', $value)) {
				$field->setValue($value['time_period']);
			}
			...
			$value['time_period'] = $field->getValue();
			$this->time_period_fields[] = $field;
		}
		unset($value);
	...
	public function toApi(array &$widget_fields = []): void {
		...
		foreach ($this->getValue() as $column_index => $value) {
			...
			foreach ($value['thresholds'] ?? [] as $threshold_index => $threshold) {
				foreach (['color', 'threshold'] as $field) {
					$widget_fields[] = [
						'type' => ZBX_WIDGET_FIELD_TYPE_STR,
						'name' => $this->name.'thresholds.'.$column_index.'.'.$field.'.'.$threshold_index,
		...
		foreach ($this->time_period_fields as $field) {
	...
	protected function getValidationRules(bool $strict = false): array {
		$rules = ['type' => API_OBJECTS, 'fields' => [
			...
			'data' => ['type' => API_INT32, 'flags' => API_REQUIRED,
				'in' => implode(',', [self::DATA_ITEM_VALUE, self::DATA_HOST_NAME, self::DATA_TEXT])],
			...
			'aggregate_function' => ['type' => API_INT32,
				'in' => implode(',', [AGGREGATE_NONE, AGGREGATE_MIN, AGGREGATE_MAX, AGGREGATE_AVG,
					AGGREGATE_COUNT, AGGREGATE_SUM, AGGREGATE_FIRST, AGGREGATE_LAST]),
				'default' => AGGREGATE_NONE],
			'time_period' => ['type' => API_ANY],
			'display' => ['type' => API_MULTIPLE, 'rules' => [
				['if' => ['field' => 'data', 'in' => self::DATA_ITEM_VALUE], 'type' => API_INT32,
					'default' => self::DISPLAY_AS_IS,
					'in' => implode(',', [self::DISPLAY_AS_IS, self::DISPLAY_BAR, self::DISPLAY_INDICATORS])],
				['else' => true, 'type' => API_INT32]
			]],
			'history' => ['type' => API_MULTIPLE, 'rules' => [
				['if' => ['field' => 'data', 'in' => self::DATA_ITEM_VALUE], 'type' => API_INT32,
					'default' => self::HISTORY_DATA_AUTO,
					'in' => implode(',', [self::HISTORY_DATA_AUTO, self::HISTORY_DATA_HISTORY,
						self::HISTORY_DATA_TRENDS])],
				['else' => true, 'type' => API_INT32]
			]],
			...
			'decimal_places' => ['type' => API_INT32, 'in' => '0:10',
				'default' => self::DEFAULT_DECIMAL_PLACES],
			...
			'text' => ['type' => API_MULTIPLE, 'rules' => [
				['if' => ['field' => 'data', 'in' => self::DATA_TEXT], 'type' => API_STRING_UTF8,
					'flags' => API_REQUIRED | API_NOT_EMPTY, 'length' => 255],
				['else' => true, 'type' => API_STRING_UTF8]
			]]
		]];
